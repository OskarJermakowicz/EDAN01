...
public class SimpleSplitDFS2 {
    ...
    public SimpleSplitDFS2(Store s, int vs) {
        store = s;
    ...
    public class ChoicePoint {
        ...
        /**
         * Value selection: (max + min) / 2 (plus extra check if it is divisble by 2
         */
        int selectValue(IntVar v) {
             if ((v.max() + v.min()) % 2 == 0) {
                return (v.max() + v.min()) / 2;
            } else {
                return (v.max() + v.min()) / 2 + 1;
            }
        }
